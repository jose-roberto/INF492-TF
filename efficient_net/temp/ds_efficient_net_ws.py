...
# Descongelar as últimas 2 camadas (+ classificador)
for param in list(net.parameters())[-30:]:
    param.requires_grad = True

# Substituir classificador
net.fc = nn.Sequential(
    nn.Dropout(0.5),
    nn.Linear(net.fc.in_features, setup['num_classes'])
)
...
def train(net, train_dataloader, test_dataloader, device):
    ...
    optimizer = torch.optim.Adam([
        {'params': list(net.parameters())[:-30], 'lr': setup['lr']/10},  # Camadas congeladas
        {'params': list(net.parameters())[-30:], 'lr': setup['lr']}       # Camadas descongeladas
...
def verify_labels(dataloader, num_samples=5):
    print("\nVerificando labels:")
...
if __name__ == "__main__":
    ...
    train_dataloader = make_dataloader(train=True, filenames="data-{000000..000010}.tar")

    test_dataloader = make_dataloader(train=False, filenames="data-{000000..000005}.tar")

    # Verificar labels antes do treino
    verify_labels(train_dataloader)
    verify_labels(test_dataloader)
    
    # Usar mixed-precision para acelerar treino
